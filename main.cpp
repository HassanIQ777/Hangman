...
void startup_animation() {
    ...
    for (int i = 0; i < 7; i++) {
        ...
        print_state(i);
        cout.flush();
        this_thread::sleep_for(chrono::milliseconds(cooldown));
        cout.flush();
    }
} //future feeture
bool isValid(string computer_choice) {
    for(char c : computer_choice) {
        if (c == ' ' || c == '-') {
            return false;
        }
    }
...
int main() {
    ...
    srand(time(0));
    string used_letters = "";
    string computer_choice  = wordlist[rand() % seize];

    do {
        computer_choice = wordlist[rand() % seize];
    ...
    for (int i = 0; i < computer_choice.size(); ++i) {
        if (rand() % 2 == 1) {
            if (rand() % 3 == 0 || rand() % 3 == 1) { // 66% chance a word is revealed :)
    ...
    while (display_string != computer_choice) {
        cout<<"\nGuess the word: " <<display_string<<" ("<<computer_choice.size()<<" characters)\n";
        ...
        if (isalpha(guess)) {
            ...
            used_letters += " ";
            //sort(used_letters.begin(), used_letters.end()); //for the time being i won't sort these neither will i do union(); to remove the repetitions
        }
        else {
